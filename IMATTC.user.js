...
// @updateURL   https://softspot.nl/ingress/plugins/iitc-plugin-imattc.meta.js
// @downloadURL https://softspot.nl/ingress/plugins/iitc-plugin-imattc.user.js
...
var imattcversion = "1.11.0.20220606.235700";
var changelog = `
Changelog:
...
- added about IMATTC message at the about screen and an about link at the bottom of the missions screen
...
function init() {
  ...
  function clickButton2CheckedMissionsList(missionScope,lastonefailed) {
      ...
      if (!resultsdiv) {
          ...
          if (document.querySelector('.navbar')) document.querySelector('.navbar').after(resultsdiv);
      }
      try {
          ...
          console.log("FAILED: change missions failed, cancel all");
          resultsdiv.innerText = 'FAILED: change missions failed, cancel all';
          localStorage.removeItem('clickbutton2missionguids');
          return false;
      }
  };
  ...
  missionScope.distance = function(lat1, lon1, lat2, lon2, unit) {
	if ((lat1 == lat2) && (lon1 == lon2)) {
      ...
      missionScope.submitMission = function(mission) {
          ...
          addEventListener(document.body, 'DOMNodeInserted', (e, closeListener) => {
  ...
  missionScope.exportImages = function(category) {
      missionScope.getFullMissionData(missionScope.categorisedMissions[category]).then(function(missions){
          ...
          for (let missioncnt in missions) {
              ...
              imagelist.push({url:mission.logo_url,filename:mission.name.replace(/[\W ]+/g,'_')});
          }
  ...
  missionScope.exportJSON = function(category) {
      missionScope.getFullMissionData(missionScope.categorisedMissions[category]).then(function(missions){
          ...
          let now = new Date();
          let timestamp = now.getFullYear() + leadingzero(now.getDate()) + leadingzero(now.getMonth()) + '_' + leadingzero(now.getHours()) + leadingzero(now.getMinutes());
          let filename = timestamp + '_' + missionScope.categoryContent[category].name.replace(/[\W ]+/g,'_') + '-mission-data.json';
      ...
      if (missionScope.categoryContent.length > 0) {
        ...
        for (var i = 0; i < missionScope.categoryContent.length; i++) {
          ...
          if (!missionScope.categoryContent[i].missions || missionScope.categoryContent[i].missions.length == 0) {
            ...
            missionContent += "<button class='btn btn-default'style='float: right!important;margin: 5px;' ng-click='exportImages(" + i + ")'>Download Images</button>"
  ...
  w.$injector.invoke(function($compile) {
    ...
    buttonContent += "<button ng-click='toggleCheckboxes()' class='yellow togglecheckboxbutton'>Show Checkboxes</button>";
...
//setTimeout(() => {
//  init();
//}); // run a little later then version 1.8.1, to be able to replace conflicting functions
...
(function() {
    ...
    .preview-mission .body-panel .panel-container .map {
        position: absolute;
        left: 0;
        right: 450px;
        top: 0;
    ...
    .preview-mission .body-panel .panel-container .waypoints {
        position: absolute;
        width: 420px;
        right: 0;
        top: 0;
    ...
    .preview-mission .angular-google-map-container {
        width: 100%;
        height: 100%;
    }
    .preview-mission .body-panel .panel-container .category-dropdown {
        bottom: 0px;
        right: 0px;
        position: absolute;
    }
